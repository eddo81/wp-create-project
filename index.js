...
const run = async () => {
	...
	do {
		...
		const answers = await prompts(
			[
...

				{
					type: "confirm",
					name: "webpack",
					message: "Use webpack to compile front-end assets?",
					initial: true
				},

				{
					type: (prev, values) => (values.webpack === true) ? "confirm" : null,
					name: "tailwind",
					message: (prev, values) => `Add Tailwind.css to this ${values.projectType}?`,
					initial: true
				},
		...
		data.webpack              = answers.webpack;
		data.tailwind             = answers.tailwind;
	...
	try {
		...
		if (data.webpack === false) {
			fs.mkdirSync(`${folders.output}/assets/js`, { recursive: true });
			fs.mkdirSync(`${folders.output}/assets/css`, { recursive: true });
			fs.mkdirSync(`${folders.output}/assets/fonts`, { recursive: true });
			fs.mkdirSync(`${folders.output}/assets/img`, { recursive: true });

			copyTpl(`${folders.input['css']}/_index.css.ejs`, `${folders.output}/assets/css/${data.projectType}.css`, data);
			copyTpl(`${folders.input['js']}/_index.js.ejs`, `${folders.output}/assets/js/${data.projectType}.js`, data);

			if(data.projectType === 'plugin') {
				copyTpl(`${folders.input['css']}/_admin.css.ejs`, `${folders.output}/assets/css/admin.css`, data);
				copyTpl(`${folders.input['js']}/_admin.js.ejs`, `${folders.output}/assets/js/admin.js`, data);
			}
		}
